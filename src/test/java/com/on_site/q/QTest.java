...
public class QTest extends TestBase {
    ...
    @Test
    public void selectorElementConstructor() throws Exception {
        Q q = $("sub", $(document("<test><sub /><sub>content</sub></test>")).get(0));
        assertSelectedSize(q, 2);
        assertNodeName(q.get(0), "sub");
        assertNodeName(q.get(1), "sub");
        assertNodeText(q.get(0), "");
        ...
        q = $("sub", $("sub", document("<test><sub /><sub>content</sub></test>")).get(1));
        assertSelectedSize(q, 0);

        q = $("sub", $("sub", document("<test><sub /><sub>inside: <sub>nested</sub></sub></test>")).get(1));
        assertSelectedSize(q, 1);
        assertNodeName(q.get(0), "sub");
